...
- Central app versioning through `version.properties` (`2.1.0`, versionCode `20100`).
...
5. The workflow runs unit tests and builds `flashcard-2.1.0-debug.apk` automatically.
...
- `flashcard-2.1.0-release.apk` — signed production APK
- `mapping-release.txt` — R8 mapping file; keep it for crash-symbolication/debugging
- `flashcard-2.1.0-debug.apk` — test APK
...
Backups created by version 2.0.0 remain compatible with 2.1.0 because the Room schema and backup format are unchanged.
